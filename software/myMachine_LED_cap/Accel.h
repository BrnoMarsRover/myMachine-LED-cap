...
public:
    ...
    // Dominantní směr náklonu
    TiltDir getTiltDir(float threshold = 2.0f) const {
        float absX = abs(_ax);
        float absY = abs(_ay);
        if (absX < threshold && absY < threshold) return TiltDir::NONE;
        if (absX >= absY) {
            return (_ax > 0) ? TiltDir::RIGHT : TiltDir::LEFT;
        }
